...
def getMinDep(data, target, x, CPC, alpha, is_discrete, dict_cache):
    ...
    for i in range(k_length+1):
        ...
        for S in SS:
            ci_number += 1
            pval, dep, dict_cache = CCT_select(data, target, x, S, is_discrete, dict_cache)
            ...
            if pval > alpha:
                return 0, S, ci_number, dict_cache
            if dep_min > dep:
                dep_min = dep
    return dep_min, None, ci_number, dict_cache


def MMPC(data, target, alpha, is_discrete, dict_cache):
    ...
    while True:
        ...
        for x in M_variables:
            ...
            x_dep_min, sepset_temp, ci_num2, dict_cache = getMinDep(data, target, x, CPC, alpha, is_discrete, dict_cache)
            ci_number += ci_num2
    ...
    for a in CPC_temp:
        ...
        for length in range(C_length+1):
            ...
            for S in SS:
                ci_number += 1
                pval, dep, dict_cache = CCT_select(data, target, a, S, is_discrete, dict_cache)
                if pval > alpha:
                    CPC.remove(a)
                    breakFlag = True
                    break

    return list(set(CPC)), sepset, ci_number, dict_cache
...
#     select_time += dic["cache"][0]
#     all_time += dic["cache"][0]+dic["cache"][1]
# end_time = time.process_time()
#
# print("time is: ", end_time - start_time)
# print(select_time / all_time)
