    ...
    for i in range(len(PCT)):
        for j in range(len(PCT)):
            if i != j:
                ...
                pval, _, dict_cache = CCT_select(Data, x, y, Z, is_discrete, dict_cache)
                num_ci += 1
                ...
                pval2, _, dict_cache = CCT_select(Data, x, y, condition_vars, is_discrete, dict_cache)
                num_ci += 1
        ...
        if IDT[T, x] == 1:
            for j in range(idT3_count):
                if idT3[j][0] == x:
                    ...
                    y = idT3[j][0]
                    IDT[T, y] = 2
    return IDT, idT3, idT3_count, idT4, idT4_count, num_ci, dict_cache
