...
def getPCD(data, target, alaph, is_discrete, dict_cache):
    ...
    while True:
        ...
        for vari in CanPCD_temp:
            ...
            for j in range(Plength+1):
                ...
                for S in SSubsets:
                    ci_number += 1
                    pval_gp, dep_gp, dict_cache = CCT_select(data, target, vari, S, is_discrete, dict_cache)
        ...
        if variDepSet != []:
            ...
            while pcd_index >=0:
                ...
                for j in range(Slength+1):
                    ...
                    for S in SSubsets:
                        ci_number += 1
                        pval_sp, dep_sp, dict_cache = CCT_select(data, target, x, S, is_discrete, dict_cache)
            ...
            break
    return list(set(PCD)), sepset, ci_number, dict_cache


def getPC(data, target, alaph, is_discrete, dict_cache):
    ci_number = 0
    PC = []
    PCD, sepset, ci_num2, dict_cache = getPCD(data, target, alaph, is_discrete, dict_cache)
    ci_number += ci_num2
    for x in PCD:
        variSet, _, ci_num3, dict_cache = getPCD(data, x, alaph, is_discrete, dict_cache)
        ci_number += ci_num3
