...
from Application.MBs.STMB import STMB
from Application.MBs.MMMB.MMMB import MMMB


def MBGSL(data, alpha, is_discrete, selected, dict_cache):
    ...
    for i in range(kvar):
        if selected == 1:
            MB, n_c, dict_cache = MMMB(data, i, alpha, is_discrete, dict_cache)
        elif selected == 2:
            MB, n_c, dict_cache = HITON_MB(data, i, alpha, is_discrete, dict_cache)
        elif selected == 3:
            MB, n_c, dict_cache = semi_HITON_MB(data, i, alpha, is_discrete, dict_cache)
        else:
            MB, n_c, dict_cache = PCMB(data, i, alpha, is_discrete, dict_cache)
        num_CI += n_c
        for j in MB:
            PP[i, j] = 1
    ...
    return pdag, num_CI, dict_cache

# import warnings
# warnings.filterwarnings('ignore')
# import pandas as pd
#
# # "D:/data/alarm1_data/Alarm1_s5000_v3.csv"
# data = pd.read_csv("D:/data/child_data/Child_s5000_v1.csv")
# print("the file read")
# import numpy as np
# num1, kvar = np.shape(data)
# alpha = 0.01
#
# pdag, dic = MBGSL(data, alpha, True, 1)
# print(pdag)
# for i in range(kvar):
#     for j in range(kvar):
#         if pdag[i, j] == -1:
